...
Bucket, catalogue caveats, partition-path format, and the full pipeline command reference now
live in [`pipeline/CLAUDE.md`](./pipeline/CLAUDE.md) — it loads automatically whenever a session
works under `pipeline/`, and is irrelevant noise for frontend-only sessions otherwise.
...
Frontend needs Node `^22.18.0 || >=24.12.0` (see `apps/web/package.json` engines).
Pipeline-specific Python environment notes live in [`pipeline/CLAUDE.md`](./pipeline/CLAUDE.md).
...
Pipeline-stage-by-stage commands (fetch, baseline, detect, score) live in
