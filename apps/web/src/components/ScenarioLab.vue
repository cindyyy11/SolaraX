...
const emit = defineEmits<{ change: [{ id: string; label: string; severity: number; duration: number; result: ScenarioResult }] }>()
...
watch(output, (result) => emit('change', { id: scenario.value.id, label: scenario.value.title, severity: severity.value, duration: duration.value, result }), { immediate: true })
